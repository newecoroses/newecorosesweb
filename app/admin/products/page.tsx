...
const RELATIONSHIP_OPTIONS = ['Him', 'Her', 'Kids', 'Friend', 'Girlfriend', 'Boyfriend', 'Wife', 'Husband', 'Family', 'Corporate', 'Couple'];
const CELEBRATION_OPTIONS = ['Birthday', 'Anniversary', 'Eid Ul Fitr', 'Navratri', 'Husband Appreciation Day', 'Holi', "Women's Day", 'Corporate Events', 'Festive Events', 'Office Celebrations'];
...
const DEFAULT_FORM: ProductFormData = {
    name: '', slug: '', description: '', collection_slugs: [],
...
// Checkbox grid component for multi-select
function CheckboxGrid({ label, options, selected, onChange }: {
    label: string;
    options: string[];
    selected: string[];
    ...
    return (
        <div>
            <label className="text-gray-400 text-xs uppercase tracking-wider font-medium block mb-2">{label}</label>
                ...
                {options.map(option => {
                    ...
                    return (
                        <button
                            ...
                            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${isChecked
...
export default function AdminProductsPage() {
    ...
    const openEdit = (p: DBProduct) => {
        setEditingProduct(p);
        // Parse collection_name which may be comma-separated for multi-collection
        const existingSlugs = (p.collection_slug ?? '').split(',').map(s => s.trim()).filter(Boolean);
        setForm({
            name: p.name,
            slug: p.slug,
            description: p.description ?? '',
            collection_slugs: existingSlugs.length > 0 ? existingSlugs : [],
    ...
    const handleSave = async () => {
        ...
        // Build comma-separated collection names and slugs from selected checkboxes
        const selectedCollections = collections.filter(c => form.collection_slugs.includes(c.slug));
        ...
        <div className="max-w-7xl mx-auto">
            ...
            <Modal open={modalOpen} onClose={() => setModalOpen(false)}>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="col-span-2">
                        <CheckboxGrid
                            label="Collections (select one or more)"
                            options={collections.map(c => c.name)}
                            selected={form.collection_slugs.map(slug => collections.find(c => c.slug === slug)?.name ?? slug)}
                            onChange={(selectedNames) => {
                                const slugs = selectedNames.map(name => collections.find(c => c.name === name)?.slug ?? '').filter(Boolean);
                                setForm(f => ({ ...f, collection_slugs: slugs }));
                            }}
                        />
                    </div>
                    ...
                    <div className="col-span-2 bg-gray-900 border border-gray-800 rounded-xl p-4">
                        ...
                        <div className="flex flex-wrap gap-4 mb-4">
                            {form.images.map((img, idx) => (
                                <div key={idx} className={`relative w-24 h-24 rounded-xl overflow-hidden bg-gray-800 border-2 border-transparent hover:border-zinc-400 transition-all group ${img.startsWith('HIDDEN::') ? 'opacity-50 grayscale' : ''}`}>
                                    ...
                                    <button type="button" onClick={() => toggleImageHidden(idx)}
                        ...
                        <div className="text-xs text-gray-500 leading-tight">
                            The first image is the main display. Any additional images will appear in the premium hover-slider on the website.
                        </div>
                    </div>

                    {/* Relationships — Checkbox grid */}
                    <div className="col-span-2">
                        <CheckboxGrid
                            label="Show in Relationships"
                            options={RELATIONSHIP_OPTIONS}
                            selected={form.relationships}
                            onChange={(sel) => setForm(f => ({ ...f, relationships: sel }))}
                        />
                    </div>

                    {/* Celebrations — Checkbox grid */}
                    <div className="col-span-2">
                        <CheckboxGrid
                            label="Show in Celebrations"
                            options={CELEBRATION_OPTIONS}
                            selected={form.celebrations}
                            onChange={(sel) => setForm(f => ({ ...f, celebrations: sel }))}
                        />
                    </div>
