                ...
                <div className="space-y-2">
                    {['All', ...collections.map((c) => c.name)].map((cat) => (
                        <button
                            ...
                            className={`block w-full text-left px-4 py-2.5 rounded-lg text-sm transition-all duration-300 ${activeCollection === cat ? 'bg-primary text-white font-medium' : 'text-muted hover:bg-secondary hover:text-foreground'}`}
                        >
                            {cat}
